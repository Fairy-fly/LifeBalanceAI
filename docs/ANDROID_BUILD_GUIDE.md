...
本文档说明如何使用 Qt 构建并部署 LifeBalanceAI Android 版本。
...
2. 选择 Android Qt Kit，例如 `Qt 6.5.3 Clang arm64-v8a`。
3. Configure 项目。
...
- Android 端读不到 `.env`：安装 Debug 包后重新执行 `tools/android_push_env.ps1`。
- 桌面端显示正常但真机布局异常：以 Android 真机截图为准继续微调安全区、密度和滚动区域。
