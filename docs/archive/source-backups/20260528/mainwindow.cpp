        ...
        if (p4) {
            ...
            if (p4Layout) {
                ...
                if (!p4->findChild<BottomNavBar *>(QStringLiteral("androidBottomNavHome"))) {
                    ...
                    homeNav->setFixedHeight(64);
                    QTimer::singleShot(0, [homeNav, p4]() {
                        homeNav->setGeometry(0, p4->height() - 64, p4->width(), 64);
                        homeNav->raise();
                    });
...
bool MainWindow::eventFilter(QObject *watched, QEvent *event)
{
    ...
    // Keep bottom nav bars pinned to bottom of their pages
    if (event && event->type() == QEvent::Resize) {
        if (watched == ui->page_4) {
            auto *re = static_cast<QResizeEvent *>(event);
            if (auto *homeNav = ui->page_4->findChild<BottomNavBar *>(QStringLiteral("androidBottomNavHome")))
                homeNav->setGeometry(0, re->size().height() - 64, re->size().width(), 64);
        }
        if (watched == ui->page_5) {
            auto *re = static_cast<QResizeEvent *>(event);
            if (auto *profileNav = ui->page_5->findChild<BottomNavBar *>(QStringLiteral("androidBottomNavProfile")))
                profileNav->setGeometry(0, re->size().height() - 64, re->size().width(), 64);
        }
    }
...
void MainWindow::applyWarmVisualPolish()
{
    ...
    if (ui->page_4) {
        if (auto *layout = qobject_cast<QVBoxLayout *>(ui->page_4->layout())) {
            ...
            int bottomMargin = 6;
            if (auto *homeNav = ui->page_4->findChild<BottomNavBar *>(QStringLiteral("androidBottomNavHome")))
                bottomMargin += homeNav->minimumHeight();
        ...
        for (QTextEdit *textEdit : ui->tabWidgetDays->findChildren<QTextEdit *>()) {
            ...
            if (textEdit->viewport()) {
                textEdit->viewport()->setAttribute(Qt::WA_AcceptTouchEvents, true);
                QScroller::grabGesture(textEdit->viewport(), QScroller::TouchGesture);
                QScroller *s = QScroller::scroller(textEdit->viewport());
...
void MainWindow::updateGoalCollapseState()
{
...
#ifdef Q_OS_ANDROID
    const int scrollMaxH = m_goalExpanded ? 108 : 0;
    const int frameMaxH  = m_goalExpanded ? 174 : 62;
#else
    const int scrollMaxH = m_goalExpanded ? 184 : 0;
    const int frameMaxH  = m_goalExpanded ? 286 : 120;
#endif
    ...
    if (!goalScroll) {
        ui->frameGoal->setMinimumHeight(frameMaxH);
        ui->frameGoal->setMaximumHeight(frameMaxH);
        ui->frameGoal->setSizePolicy(QSizePolicy::Expanding, QSizePolicy::Fixed);
        return;
    }

    // Skip animation if widget hasn't been laid out yet
    const int curScrollH = goalScroll->height();
    if (curScrollH < 10) {
        goalScroll->setVisible(m_goalExpanded);
        goalScroll->setMinimumHeight(scrollMaxH);
        goalScroll->setMaximumHeight(scrollMaxH);
        goalScroll->setSizePolicy(m_goalExpanded ? QSizePolicy::Expanding : QSizePolicy::Ignored,
    ...
    // Animate only the content scroll area height
    if (m_goalExpanded)
        goalScroll->setVisible(true);

    // Allow frameGoal to grow during animation
    ui->frameGoal->setMaximumHeight(QWIDGETSIZE_MAX);
    ui->frameGoal->setSizePolicy(QSizePolicy::Expanding, QSizePolicy::Preferred);
    ...
    connect(anim, &QVariantAnimation::valueChanged, this, [=](const QVariant &val) {
        float t = val.toFloat();
        if (scrollPtr) {
            int h = qRound(curScrollH + (scrollMaxH - curScrollH) * t);
    ...
    connect(anim, &QVariantAnimation::finished, this, [=]() {
        if (scrollPtr) {
            scrollPtr->setMinimumHeight(scrollMaxH);
            scrollPtr->setMaximumHeight(scrollMaxH);
...

{
    ...
    // Request AI-generated nickname if user did not provide one
    if (nickname.isEmpty()) {
        QString profileSummary = buildUserProfileString();
...

{
    ...
    if (jsonResult.isEmpty()) {
        ...
        if (m_goalContentEdit)
            m_goalContentEdit->setPlainText(tr("（计划生成超时，请稍后重试）"));
    ...
    if (doc.isNull() || !doc.isObject()) {
        ...
        if (m_goalContentEdit)
            m_goalContentEdit->setPlainText(tr("（计划生成失败，请重试）"));
...

{
    ...
    if (page5Layout) {
        ...
        if (!profileNav) {
            ...
            QTimer::singleShot(0, [profileNav]() {
                QWidget *p5 = profileNav->parentWidget();
                if (p5) {
                    profileNav->setGeometry(0, p5->height() - 64, p5->width(), 64);
                    profileNav->raise();
                }
            });
