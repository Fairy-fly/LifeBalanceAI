...
#ifdef Q_OS_ANDROID
    {
        // Lightweight scale-up animation: 0.85 → 1.0, 100ms, OutCubic
        QPoint end = pos();
        QSize endSize = size();
        QPoint adjustedPos = end + QPoint(
            static_cast<int>(endSize.width() * 0.075),
            static_cast<int>(endSize.height() * 0.075)
        );
        setWindowOpacity(1.0);
        show();

        auto *group = new QParallelAnimationGroup(this);
        auto *geom = new QPropertyAnimation(this, "geometry", group);
        QRect startGeom(adjustedPos, endSize * 0.85);
        QRect endGeom(end, endSize);
        geom->setDuration(100);
        geom->setStartValue(startGeom);
        geom->setEndValue(endGeom);
